...
public:
    ...
    void compute(const std::function<std::vector<double>(const std::string&, int)>& builder) override {
        int total_window = window + shift;
        auto bv1_sum = builder("bv1_sum", total_window);
        auto sv1_sum = builder("sv1_sum", total_window);
        auto b1 = builder("b1", total_window);
        auto s1 = builder("s1", total_window);
