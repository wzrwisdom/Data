...
public:
    ...
    void compute(const std::function<std::vector<double>(const std::string&, int)>& builder) override {
        auto buy_v = builder("en_buy_vol", window);
        auto buy_p = builder("en_buy_price", window);
        auto sell_v = builder("en_sell_vol", window);
        auto sell_p = builder("en_sell_price", window);
