...
class EnSellSumPriceRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        window_tot = window
        en_sell_p = builder(name="s1", window=window_tot)
        en_sell_v = builder(name="sv1_sum", window=window_tot)
...
class MACDFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        last = builder(name="last", window=None)
        res = ta.MACD(last)[0]
        return res.iloc[-1].item() if not np.isnan(res.iloc[-1]) else None
...
class CLHDELTAFACTOR(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        last = builder(name="last", window=window_tot)
...
class RetVolCorrFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        last = builder(name="last", window=window+1)
        vol = builder(name="vol", window=window)
...
class TradePVRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        vol = builder(name="vol", window=window-1)
...
class TradeVolInHihgPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        vol = builder(name="vol", window=window)
        price = builder(name="last", window=window_tot)
        if len(price) < window:
            return 0
...
class TradeRetVProdFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        vwap = builder(name="vwap", window=window + 1)
        vol = builder(name="vol", window=window)
...
class PriceVolCorrFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        last = builder(name="last", window=window)
        vol = builder(name="vol", window=window)
...
class BSPowerFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        buy_v = builder(name="bv1_sum", window=window)
        buy_p = builder(name="b1", window=window)
        sell_v = builder(name="sv1_sum", window=window)
        sell_p = builder(name="s1", window=window)
        last = builder(name="last", window=1)
...
class EnBuyPriceStdFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        en_buy_p = builder(name="b1", window=window_tot)
...
class BSPV1TsrankFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        b1 = builder(name="b1", window=window)
        s1 = builder(name="s1", window=window)
        bv1 = builder(name="bv1_sum", window=window)
        sv1 = builder(name="sv1_sum", window=window)
...
class ReturnVolProductFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        close = builder(name="last", window=window + 1)
        vol = builder(name="vol", window=window)
