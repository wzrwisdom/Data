...
class CloseFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        close = builder(name="close", window=shift + 1)
...
class ReturnVolProductFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        close = builder(name="close", window=window + 1)
        vol = builder(name="td_vol", window=window)
        ...
        #     # 计算收益率
        #     values = [
        #         (close.iloc[i + 1] - close.iloc[i]) / close.iloc[i] * vol.iloc[i]
...
class WB1Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        wb1 = builder(name="wb1", window=window)
...
class WB10Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        wb10 = builder(name="wb10", window=window)
...
class EnSellSumPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        en_sell_p = builder(name="en_sell_price", window=window_tot)
        en_sell_v = builder(name="en_sell_vol", window=window_tot)
        avg_p = builder(name="bs_avg_price", window=window1)

        if len(en_sell_p) < int((window1+window2)/3)-1:
            return None

        sell_avg_p = avg_price(en_sell_p.ffill(), en_sell_v, window2)[-window1:]
...
class EnBuySumPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        window2 = self.kwargs["window2"]
        window_tot = window1 + window2 - 1
        en_buy_p = builder(name="en_buy_price", window=window_tot)
        en_buy_v = builder(name="en_buy_vol", window=window_tot)
        avg_p = builder(name="bs_avg_price", window=window1)
...
class OBPriceSpreadFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        s1 = builder(name="s1", window=window)
        b1 = builder(name="b1", window=window)
...
class BSv5Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        bv5_sum = builder(name="bv5_sum", window=window)
        sv5_sum = builder(name="sv5_sum", window=window)
...
class DeridBid1Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        shift = self.kwargs["shift"]
        b1 = builder(name="b1", window=shift + 1)
        if len(b1) < shift + 1:
            return None
...
class EnSellPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        s1 = builder(name="s1", window=window)
        en_sell_p = builder(name="en_sell_price", window=window)
        avg_p = builder(name="bs_avg_price", window=window)
        ...
        s1 = s1.round(4)
        avg_p = avg_p.round(4)
        res = ts_rank(en_sell_p.ffill() - avg_p, window=window)
        return res.iloc[-1].item() if not np.isnan(res.iloc[-1]) else None
...
class DEMAFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        close = builder(name="close", window=None)
...
class BestVImbalanceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        bv = builder(name="bv1_sum", window=window_tot)
        sv = builder(name="sv1_sum", window=window_tot)
        b1 = builder(name="b1", window=window_tot)
        s1 = builder(name="s1", window=window_tot)
...
#         high = builder(name="high", window=window_tot)
#         low = builder(name="low", window=window_tot)
#         if len(low) < window:
#             return None
#
...
class EnBSPriceDiffFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        en_buy_p = builder(name="en_buy_price", window=1)
        en_sell_p = builder(name="en_sell_price", window=1)
        b1 = builder(name="b1", window=1)
        s1 = builder(name="s1", window=1)
        avg_p = builder(name="bs_avg_price", window=1)
        res = (en_buy_p.ffill() - en_sell_p.ffill()) / avg_p
        if len(res) < 1:
            return None
        return res.iloc[-1].item() if not np.isnan(res.iloc[-1]) else None
...
class EnSellRet10Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        window = self.kwargs["window"]
        window_tot = shift + window
        ssv10_sum = builder(name="ssv10_sum", window=window_tot)
        sv10_sum = builder(name="sv10_sum", window=window_tot)
...
class EnBuyRet10Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        window = self.kwargs["window"]
        window_tot = shift + window
        bbv10_sum = builder(name="bbv10_sum", window=window_tot)
        bv10_sum = builder(name="bv10_sum", window=window_tot)
...
class EnSellP10Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        ssv10_sum = builder(name="ssv10_sum", window=window)
        sv10_sum = builder(name="sv10_sum", window=window)
...
class CanEnVolRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        en_sell_v = builder(name="en_sell_vol", window=window)
        en_buy_v = builder(name="en_buy_vol", window=window)
        can_sell_v = builder(name="cancel_sell_vol", window=window)
        can_buy_v = builder(name="cancel_buy_vol", window=window)
...
class OBPriceDeriFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        avg_p = builder(name="bs_avg_price", window=window_tot)
...
class EnBuyPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        en_buy_p = builder(name="en_buy_price", window=window)
        b1 = builder(name="b1", window=window)
        avg_p = builder(name="bs_avg_price", window=window)
        ...
        res = ts_rank(en_buy_p.ffill() - avg_p, window)
        return res.iloc[-1].item() if not np.isnan(res.iloc[-1]) else None
...
class EnBuyP10Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        bbv10_sum = builder(name="bbv10_sum", window=window)
        bv10_sum = builder(name="bv10_sum", window=window)
...
class RetNCorrFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        close = builder(name="close", window=window+1)
        td_buy_n = builder(name="td_buy_num", window=window)
        td_sell_n = builder(name="td_sell_num", window=window)
...
class TradeVolRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        td_buy_v = builder(name="td_buy_vol", window=window)
        td_sell_v = builder(name="td_sell_vol", window=window)
...
class EnSellRet5Factor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        window = self.kwargs["window"]
        window_tot = shift + window
        ssv5_sum = builder(name="ssv5_sum", window=window_tot)
        sv5_sum = builder(name="sv5_sum", window=window_tot)
...
class OHLCRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        o = builder(name="open", window=window)
        c = builder(name="close", window=window)
        h = builder(name="high", window=window)
        l = builder(name="low", window=window)
...
class TradePVRatioFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        vwap = builder(name="vwap", window=window)
        vol = builder(name="td_vol", window=window-1)
...
class TradeVolInHihgPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        td_buy_v = builder(name="td_buy_vol", window=window)
        td_sell_v = builder(name="td_sell_vol", window=window)
        price = builder(name="close", window=window_tot)
        if len(price) < window:
            return 0
...
class HCPFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        close = builder(name="close", window=window)
        td_buy_p = builder(name="td_buy_price", window=window)
...
class LCPFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        close = builder(name="close", window=window)
        td_sell_p = builder(name="td_sell_price", window=window)
...
class TradePowerFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        buy_v = builder(name="td_buy_vol", window=window)
        buy_p = builder(name="td_buy_price", window=window)
        sell_v = builder(name="td_sell_vol", window=window)
        sell_p = builder(name="td_sell_price", window=window)
        close = builder(name="close", window=1)
...
class TradeAvgVolInHighPriceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        td_buy_n = builder(name="td_buy_num", window=window)
        td_sell_n = builder(name="td_sell_num", window=window)
        close = builder(name="close", window=window_tot)
        if len(close) < window:
            return 0
...
class EnPowerFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        buy_v = builder(name="en_buy_vol", window=window)
        buy_p = builder(name="en_buy_price", window=window)
        sell_v = builder(name="en_sell_vol", window=window)
        sell_p = builder(name="en_sell_price", window=window)
        close = builder(name="close", window=1)
...
class RetVolCorrFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        close = builder(name="close", window=window+1)
        vol = builder(name="td_vol", window=window)
        td_buy_num = builder(name="td_buy_num", window=window)
        td_sell_num = builder(name="td_sell_num", window=window)
...
class AwesomeOscillatorFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        low = builder(name="low", window=None)
        high = builder(name="high", window=None)
...
class TradePriceStdFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        ...
        vwap = builder(name="vwap", window=window_tot)
...
class CancelTimeBuyMedFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        window = self.kwargs["window"]
        ct_b_med = builder(name="cancel_buy_time_med", window=window)
...
class EnVolInbalanceFactor(Factor):
    ...
    def compute(self, builder, **kwargs):
        en_buy_v = builder(name="en_buy_vol", window=1)
        en_sell_v = builder(name="en_sell_vol", window=1)
        if len(en_buy_v) < 1:
            return None
